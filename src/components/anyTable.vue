      ...
      <table  class="myTable mt-3">
        ...
        <tr v-for="(row,rowIndex) in datasource.rows" :key="rowIndex">
            ...
            <td v-if="group">
              ...
              <mdb-btn class="px-2 py-1 m-0 ml-1" :disabled="row.opened_group_status" @click="startGroup" :data-row="rowIndex" style="font-size: 8px;" color="info">{{$t('start')}}</mdb-btn>
            </td>
            <td class="text-center">
              <i v-if="userLock" class="fas fa-user-lock text-warning mask waves-effect m-0 pr-3" :class="{'applied': disable}" v-on:click="LockRow" :data-row="rowIndex"></i>
              <i class="fas fa-edit editIcon mask waves-effect  m-0 pr-2" :class="{'applied': disable}" v-on:click="editRow" :data-row="rowIndex"></i>
              <i class="fas fa-trash-restore-alt delIcon mask waves-effect m-0 pl-2" :class="{'applied': disable}" v-on:click="deleteRow" :data-row="rowIndex"></i>
            </td>
        </tr>
        </tbody>
      </table>
  ...
  methods: {
    ...
    async checklist (option){
      ...
      paginationClick(i){
        ...
        startGroup(ev){
          this.num_target = ev.target.dataset.row;
          this.$emit('start',this.datasource.rows[this.num_target]);
        },
        ...
        LockRow(ev){
          ...
          if(this.datasource.rows[this.num_target].auth == null){
            this.$router.push('/authorization/0')
          }
          else{
            this.$router.push('/authorization/' + this.datasource.rows[this.num_target].auth.id)
          }
        ...
        upFunc(column){
          console.log('hiy')
          console.log(column)
          this.datasource.rows.sort(function(a, b){
