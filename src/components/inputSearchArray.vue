<template>
  <div class="d-flex  inputContentIcon " :class="{'activeInputContentSearch': active, 'activeInputError': error, 'inputBorderContent': !error}">
    ...
    <div class="droplistSearch card w-100 down_list" v-if="dropshow">
      <div  class="m-0  px-3 bg_light" v-for="(item,i) in list" :key="i"   @click="selectitem(item)">
        <span class="ml-1">{{item.clinet_obj.fio}}</span>
        <div class="d-flex">
          <p style="font-size:12px; color:gray;" class="m-0">{{item.clinet_obj.first_phone_number_name}}</p>
          <p style="font-size:12px; color:gray;" class="m-0 ml-3">{{item.clinet_obj.first_address_name}}</p>
        </div>
...
export default {
  ...
  methods: {
    selectitem(data){
      this.$emit('select', data.clinet_obj)
      this.name = data.clinet_obj.first_phone_number_name.slice(4,13);
      this.dropshow = false;
    ...
    pressEnter(){
      if(this.list.length>0){
        this.$emit('select', this.list[0].clinet_obj)
        this.name = this.list[0].clinet_obj.first_phone_number_name;
        this.dropshow = false;
        return true;
      }
    },
    ...
    async search(){
      setTimeout(await this.searchFunc, 1200);
    },
