export default {
    ...
    actions: {
        async fetchMeasurment(ctx) {
            const res = await fetch(ctx.rootState.hostname + '/TegirmonUnitMeasurment');
            const res_data = await res.json();
            // console.log(ctx.rootState.hostname);
            ctx.commit('updateMeasurment', res_data);
        },
        // async fetchMeasurment_column(ctx, id) {
        //     const res = await fetch(ctx.rootState.hostname + '/TexColumnConfigs/getColumnListAsJsonObj?tableName=tegirmon_unit_measurment&auth_id=' + id);
        //     const res_data = await res.json();
