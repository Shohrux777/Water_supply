  ...
  methods:{
    ...
    async sendTelegramNewClient(client){
      ...
      for(let i=0; i<client.addresses.length; i++){
        await this.sendLocationToTelegram(client.addresses[i].latidu, client.addresses[i].longitu, client.fio, 'Location');
      ...
      for(let i=0; i<client.phone_numbers_list.length; i++){
        await this.sendPhoneToTelegram(client.phone_numbers_list[i].phone_number, client.fio)
      }
    ...
    async sendLocationToTelegram(lat, long, fio, tuman){
      try{
        this.loading = true;
        const res = await fetch('https://api.telegram.org/bot5168670876:AAGp9WQY4jDrk2j_ZU57Cy3-0g6av0TE-ZA/sendVenue?chat_id=@extreme_water_clients&&latitude='+ lat +'&longitude='+ long + '&title='+ fio +'&address=' + tuman);
        // const data = await res.json();
        this.loading = false;
        if(res.status == 200 || res.status == 201){
        }
