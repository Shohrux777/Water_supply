<template>
  <div>
    ...
    <div class="border-bottom navbar_sticky">
      <router-link to="/contragent_type">
         <h5 class="m-0 ml-3 d-flex" style="padding: 14px 0px">
           <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-chevron-left mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2.5" stroke="#007BFF" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            {{$t('Add_contragent_type')}}</h5>
      </router-link>
    </div>
    <div class="contragent_type_add px-5 pt-4">
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name"  validate error="wrong" success="right" />
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/WaterContragentTypes", requestOptions);
    ...
    async submit()
      {
        if(await this.save_data())
        {
          ...
          this.$router.push("/contragent_type");
        }
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/contragent_type_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
