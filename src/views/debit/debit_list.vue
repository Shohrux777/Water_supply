      ...
      <table class="myTableDebitorList ">
        ...
        <tbody>
          <tr v-for="(row,rowIndex) in debitList" :key="rowIndex" @click="show_infoDebit(row)">
            ...
            <td> <span >{{row.client.phone_number_1}}</span> </td>
            <td> <span >{{row.client.phone_number_2}}</span> </td>
            <td> <span >{{row.client.passport_number_str}}</span> </td>

            <td> <span >{{row.real_summ}}</span> </td>
            ...
            <td class="">
              <mdb-btn class="mr-1 py-1 px-3"  style="font-size: 9px;" color="success"  @click="payDebit(row)" 
...
export default {
  ...
  methods: {
    ...
    async updateDebit(){
      this.loading = true;
      try{
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazDebit/getPaginationFullWithClient?page=0&size=500');
