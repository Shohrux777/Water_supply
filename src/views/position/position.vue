...
export default {
  ...
  computed: mapGetters(['allPosition']),
  methods: {
    ...mapActions(['fetchPosition']),
    ...mapMutations(['position_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/position_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/OquvMarkazPosition/" + del_data.id, requestOptions);
          ...
          if(response.status == 201 || response.status == 200)
          {
            ...
            this.position_delete_row(index);
          }
          else{
