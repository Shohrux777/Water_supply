<template>
  <div>
    ...
    <div class="product_add px-5 pt-4">
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name" valid  validate error="wrong" success="right" icon="dolly-flatbed"/>
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('info')}}</p>
          </div>
          <div class="col_5 px-2">
            <input-icon  v-model="info" icon="dollar-sign"/>
            <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
              {{$t('write_district_name_full')}}
            </small> -->
          </div>
         ...
         <div class="row_w mt-4">
            ...
            <div class="col_5 px-2">
              <input-icon  v-model="code" icon="code"/>
              <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
                {{$t('write_district_name_full')}}
              </small> -->
            </div>
          ...
          <div class="row_w mt-4">
            <div class="col_3">
              <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('main_product')}}</p>
            </div>
            <div class="col_5 px-2">
              <div class="custom-control custom-checkbox">
                <input type="checkbox" v-model="main_product" class="custom-control-input" id="defaultChecked2" >
                <label class="custom-control-label" for="defaultChecked2">{{$t('main_product')}}</label>
              </div>
              <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
                {{$t('write_district_name_full')}}
              </small> -->
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/WaterProducts", requestOptions);
