...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['company_delete_row']),
      ...
      for_edit(edit_data)
      {
        this.$router.push("/province_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/WaterViloyats/" + del_data.id, requestOptions);
