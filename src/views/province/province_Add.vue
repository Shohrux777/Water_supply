<template>
  <div>
    ...
    <div class="province_add px-5 pt-4">
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name"  validate error="wrong" success="right"  icon="home"/>
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/WaterViloyats/' + this.id);
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/WaterViloyats", requestOptions);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/province_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
