...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['brend_delete_row']),
      for_edit(edit_data)
      {
        this.$router.push("/room_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ArchiveRoom/" + del_data.id, requestOptions);
