...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['test_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/test_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/OquvMarkazClient/deleteOquvMarkazTest?id=" + del_data.id, requestOptions);
